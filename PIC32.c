 ...
 * Author: Utente
 *
 * Created on 13 marzo 2013, 11.56
 */
...
#pragma config FPLLIDIV = DIV_1, FPLLODIV = DIV_8, FNOSC = PRI, FSOSCEN = OFF
#pragma config IESO = OFF, POSCMOD = HS, OSCIOFNC = OFF, FPBDIV = DIV_1
#pragma config FCKSM = CSDCMD, FWDTEN = OFF, DEBUG = OFF, JTAGEN = OFF
#pragma config ICESEL = ICS_PGx4, PWP = OFF, CP = OFF, IOL1WAY = OFF

#define led LATCbits.LATC9
#define data_add_0_w LATBbits.LATB9
#define data_add_1_w LATCbits.LATC6
#define data_add_2_w LATCbits.LATC7
#define data_add_3_w LATCbits.LATC8
#define data_add_4_w LATBbits.LATB10
#define data_add_5_w LATBbits.LATB11
#define data_add_6_w LATBbits.LATB12
#define data_add_7_w LATBbits.LATB13
#define data_add_0_r PORTBbits.RB9
#define data_add_1_r PORTCbits.RC6
#define data_add_2_r PORTCbits.RC7
#define data_add_3_r PORTCbits.RC8
#define data_add_4_r PORTBbits.RB10
#define data_add_5_r PORTBbits.RB11
#define data_add_6_r PORTBbits.RB12
#define data_add_7_r PORTBbits.RB13
#define data_en LATAbits.LATA0
#define add_latch LATBbits.LATB15
#define sw0 PORTBbits.RB7
#define sw1 PORTBbits.RB6
#define sw2 PORTBbits.RB5
#define sw3 PORTCbits.RC5
#define sw4 PORTCbits.RC4
#define p0 PORTBbits.RB4
#define p1 PORTAbits.RA4
#define p2 PORTAbits.RA9
#define p3 PORTCbits.RC3

#define dig_analog_en LATBbits.LATB8

//display command
#define rs_dpy LATBbits.LATB9
#define en_dpy LATBbits.LATB11
#define db4_dpy LATBbits.LATB10
#define db5_dpy LATCbits.LATC8
#define db6_dpy LATCbits.LATC7
#define db7_dpy LATCbits.LATC6
...
int main(int argc, char** argv) {
     ...
     write_display(1);
     strcpy(st_x_dpy, "BRS PIC32MX130  "); //first message on display
     write_display(2);
     __delay_ms(1000);
     ...
     while(1){
        ...
        if(sw_selector.whole != old_sw_selector.whole){
            disable_module();
            dig_analog_en = 0;  //AN channel disable
...
//***************************ANALOGIC MODULE        **************************************************
        if(sw_selector.whole == 2){     //ANALOGIC module
          ...
          if(analog_sel.a0){              //A/D SPI throw MCP3201
            ...
            //set up
            SPI1CONbits.SMP = 0; //sample in the middle of data out
            SPI1CONbits.CKE = 1; //data transmitted on falling edge of ck
            SPI1CONbits.CKP = 0; //in idle state CK is low
            //primary and secondary prescaler as by reset (always 0)
            SPI1CONbits.MSTEN = 1; //master mode
            SPI1CONbits.MODE32 = 0; //8 bits mode
            SPI1CONbits.MODE32 = 0;
            SPI1CONbits.ON = 1; //enableSPI port
          ...
          if(analog_sel.a1){  //MCP9700 temp. sensor
              ...
              ANSELCbits.ANSC2 = 1; //input as analog
              TRISCbits.TRISC2 = 1;
              ...
              dig_analog_en = 1;  //AN channel enabled
              ...
              ad_conv_buf = ((ADC1BUF0/2)*33/50) - 50;  //convert MCP9700 output in volt 
          ...
          if(analog_sel.a2){      //VPOT
              ...
              ANSELCbits.ANSC0 = 1; //input as analog
              TRISCbits.TRISC0 = 1;
              ...
              ad_conv_buf = (ad_conv_buf*33)/50; //convert by 5V to 3V3
          ...
          if(analog_sel.a3){      //PTC
              ...
              ANSELCbits.ANSC1 = 1; //input as analog
              TRISCbits.TRISC1 = 1;
...
//*************************DAC MODULE******************************************************
        if(sw_selector.whole == 3){     //DAC module
          ...
          ANSELBbits.ANSB0 = 1; //input as analog
          TRISBbits.TRISB0 = 1;
          ...
          dig_analog_en = 1;   //enable AN channel
          ...
          //voltage on display
          ad_conv_buf = (ADC1BUF0*5)/102;  //10 bit a/d to voltage conversion
          ad_conv_buf = (ad_conv_buf*34)/50; //convert by 5V to 3V3
          itoa(st_ad, ad_conv_buf, 10);  //convertion of integer in char string
          st_x_dpy[0] = 'T';
          st_x_dpy[1] = 'r';
          st_x_dpy[2] = 'i';
          ...
          spi_buffer.whole = (ADC1BUF0*33/50)*97/20;  //for convert 10 bit a/d to 12 bit DAC
 //the conversion facto is for: convert the read from adc PIC 10 bit in 12 bit dac,
//convert the maximum output of dac (4.096V, setted with bit 13 to zero) to 5V;
//the value is: 4*5/4.096= 4.88, so 97/20; *33/50 to convert 5V in 3V3
...
//************************MEMORY MODULE*****************************************************
        ...
        if(sw_selector.whole == 5){     //OPTO module
          ...
          if(opto_0.first){
            ...
            write_display(2);
            selection_module(5);
            TRISBbits.TRISB9 = 1; //data_add_0 as input
            TRISCbits.TRISC6 = 1; //data_add_1 as input
            data_add_2_w = 0;
            data_add_3_w = 0;
            TRISCbits.TRISC7 = 0; //data_add_2 as output
            TRISCbits.TRISC8 = 0; //data_add_3 as output
            TRISBbits.TRISB10 = 1; //data_add_4 as input
            enable_module();
          }
        ...
        if(sw_selector.whole == 7){     //PWM stepper, p0 pwm p1 stepper
            ...
            while(pwm_pp.pwm){             //p0 inc, p1 dec, p2 inv, p3 exit
                if(pwm_pp.one_time){
                    ...
                    T2CONbits.ON = 0;
                    T2CONbits.TCKPS0 = 1;   //prescaler max
                    T2CONbits.TCKPS1 = 1;
                    T2CONbits.TCKPS2 = 1;
                    PR2 = 205;
                    IFS0bits.T2IF = 0;
                    IPC2 = 31;    //max priority
            ...
            if(pwm_pp.stepper){             //p0 inc speed, p1 dec speed, p3 exit
               ...
               if(pwm_pp.stepper_3&&pwm_pp.stepper_change_phase){
                ...
                if(p0&&!aux_0.old_p0&&(stepper_time > 500)){  //inc speed
...
//**************************************************************************************************
 ...
 void initialize(void){     //initialize function
    ...
    ANSELBbits.ANSB12 = 0;     //mixed analog I/O  as input or output
    ANSELBbits.ANSB13 = 0;
    ANSELBbits.ANSB15 = 0;
    ANSELAbits.ANSA0 = 0;
    ANSELBbits.ANSB14 = 0;
    ...
    TRISBbits.TRISB14 = 0;    //set SPI CK as output
    RPB1R = 0b0011;           //remappable pin 22 as SDO

    
    TRISBbits.TRISB8 = 0;   //dig_analog_en as output
    dig_analog_en = 0;

    U1RXR = 0b0100;         //U1RX as remappable pin RB2
    TRISBbits.TRISB2 = 1;
    ANSELBbits.ANSB2 = 0;
    RPB3R = 0b0001;         //RB3 remappable pin U1TX
...
void selection_module(short int add){
    ...
    TRISBbits.TRISB9 = 0;     //pin micro for selection setup as output
    TRISCbits.TRISC6 = 0;
    TRISCbits.TRISC7 = 0;
    TRISCbits.TRISC8 = 0;
