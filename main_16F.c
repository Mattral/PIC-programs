 ...
 * Author: Utente
 *
 * Created on 15 febbraio 2013, 17.26
 */
...
#define data_add_0_r RC2
#define data_add_1_r RD5
#define data_add_2_r RD6
#define data_add_3_r RD7
#define data_add_4_r RA0
#define data_add_5_r RA1
#define data_add_6_r RA2
#define data_add_7_r RA3
#define data_en LATA4
#define add_latch LATC1
#define sw0 RD3
#define sw1 RD2
#define sw2 RD1
#define sw3 RD0
...
int ad_conv_buf, mem_data, add_data, spi_buf, uart_tx, uart_rx, pwm_on, pwm_off, stepper_time;
...
int main(void) {            //main function
        ...
        write_display(1);
        strcpy(st_x_dpy, "BRS PIC16F1937  "); //first message on display
        write_display(2);
        __delay_ms(1000);
...
//***************************ANALOGIC MODULE        **************************************************
        if(sw_selector.whole == 2){     //ANALOGIC module
          ...
          if(analog_sel.a1){  //MCP9700 temp. sensor
              ...
              ADNREF = 0;  //VREF- as AVSS
              ADPREF1 = 0; //VREF+ AS AVDD
              ADPREF0 = 0;
...
//*************************DAC MODULE******************************************************
        if(sw_selector.whole == 3){     //DAC module
          ...
          TRISB4 = 1;       //select pin  as AN11 (to read the trimmer)
          ANSB4 = 1;
          ...
          __delay_ms(200);  //delay to charge capacitors on RB4 pin
        ...
        if(sw_selector.whole == 7){     //PWM stepper, p0 pwm p1 stepper
            ...
            if(pwm_pp.stepper){             //p0 inc speed, p1 dec speed, p3 exit
               if(pwm_pp.one_time){
                    ...
                    T2CKPS0 = 1;  //prescaler /64, freq 15.625KHz
                    T2CKPS1 = 1;                 

                    T2OUTPS0  = 0;  //postscaler /15, freq = 1041 Hz
                    T2OUTPS1  = 1;
                    T2OUTPS2  = 1;
                    T2OUTPS3  = 1;
